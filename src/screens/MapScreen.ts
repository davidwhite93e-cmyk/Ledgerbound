...
import { CITIES, City, getCity, distanceBetween } from "../data/cities.js";
import { FACTIONS, rankForStanding } from "../data/factions.js";

export interface MapNav {
  toCity: (state: GameState) => void;
  toTravel: (state: GameState, destId: string) => void;
}
...
export function travelCostFor(distance: number): { gold: number; days: number } {
  return { gold: Math.max(5, Math.round(distance / 4)), days: Math.max(1, Math.round(distance / 70)) };
}

export function renderMapScreen(container: HTMLElement, state: GameState, nav: MapNav): void {
  ...
  const canvas = el("canvas", { width: String(CANVAS_W), height: String(CANVAS_H), className: "map-canvas" }) as HTMLCanvasElement;
...
export function renderStatusBar(state: GameState): HTMLElement {
  ...
  const factionLine = FACTIONS.map(
    (f) => `${f.name.split(" ").pop()}: ${rankForStanding(f, state.data.factionStanding[f.id])}`
  ).join("   ");
