...
import { GameState, StatBlock, STARTING_STAT_POINTS } from "../state/GameState.js";

const STAT_LABELS: { key: keyof StatBlock; label: string; hint: string }[] = [
...
export function renderCreateScreen(
  ...
  let name = "Wanderer";
  const allocated: StatBlock = { trade: 0, combat: 0, travel: 0, defense: 0 };
  ...
  const nameInput = el("input", {
    ...
    oninput: (e: Event) => {
      name = (e.target as HTMLInputElement).value || "Wanderer";
    },
  }) as HTMLInputElement;
  ...
  const valueLabels: Partial<Record<keyof StatBlock, HTMLElement>> = {};
