...
export function renderCityScreen(container, state, nav, initialTab = "market") {
    ...
    function setTab(tab) {
        ...
        clear(content);
        if (tab === "market")
            content.append(renderMarket(state, nav));
        else
            content.append(renderGuildHall(state, nav));
    }
    setTab(activeTab);
...
function renderMarket(state, nav) {
    ...
    table.append(el("div", { className: "market-header" }, [
        el("span", {}, ["Good"]),
        el("span", {}, ["Price"]),
        el("span", {}, ["Owned"]),
        el("span", {}, ["Qty"]),
    ...
    for (const good of GOODS) {
        ...
        const row = el("div", { className: `market-row${good.contraband ? " contraband" : ""}` }, [
            ...
            el("span", { className: "qty-controls" }, [
                el("button", { className: "stepper-btn", onclick: () => { if (qty > 1) {
                        qty--;
            ...
            el("span", { className: "trade-buttons" }, [
                el("button", {
                    ...
                    onclick: () => {
                        ...
                        if (!result.ok)
                            alert(result.reason ?? "Can't buy that.");
                        else
                            refreshRow();
                    },
                }, ["Buy"]),
        ...
        const ownedSpan = row.children[2];
        function refreshRow() {
...
let statusRefreshHook = null;
export function setStatusRefreshHook(fn) {
    statusRefreshHook = fn;
}
function renderGuildHall(state, nav) {
...
function renderGuildHallRefresh(container, state, nav) {
    clear(container);
